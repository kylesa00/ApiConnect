/*
 * Webshop Service API
 *
 * Webshop services connect Webshop applications to ERP system. The entry point of Webshop API is `/customers/{companyName}/{customerNr}`, which is called by Webshop application whenever a user logs in. If the customer requestedOrderPosition by its number could be found, the response contains a `_links` section, which contains all possible navigations and actions the customer can take.
 *
 * OpenAPI spec version: 2.0-draftAv
 * 
 * Generated by: https://github.com/swagger-api/swagger-codegen.git
 */

using System;
using System.Linq;
using System.IO;
using System.Text;
using System.Collections;
using System.Collections.Generic;
using System.Collections.ObjectModel;
using System.ComponentModel.DataAnnotations;
using System.Runtime.Serialization;
using Newtonsoft.Json;

namespace IO.Swagger.Models
{ 
    /// <summary>
    /// 
    /// </summary>
    [DataContract]
    public partial class VendorStocks : IEquatable<VendorStocks>
    { 
        /// <summary>
        /// List of vendor article stocks
        /// </summary>
        /// <value>List of vendor article stocks</value>
        [DataMember(Name="stocks")]
        public List<VendorStock> Stocks { get; set; }

        /// <summary>
        /// Delivery Date in ISO 8601 format
        /// </summary>
        /// <value>Delivery Date in ISO 8601 format</value>
        [DataMember(Name="deliveryDate")]
        public DateTime? DeliveryDate { get; set; }

        /// <summary>
        /// Cutoff Time in ISO 8601 format
        /// </summary>
        /// <value>Cutoff Time in ISO 8601 format</value>
        [DataMember(Name="cutoffTime")]
        public DateTime? CutoffTime { get; set; }

        /// <summary>
        /// Error message sent by vendor
        /// </summary>
        /// <value>Error message sent by vendor</value>
        [DataMember(Name="errorMessage")]
        public string ErrorMessage { get; set; }

        /// <summary>
        /// Returns the string presentation of the object
        /// </summary>
        /// <returns>String presentation of the object</returns>
        public override string ToString()
        {
            var sb = new StringBuilder();
            sb.Append("class VendorStocks {\n");
            sb.Append("  Stocks: ").Append(Stocks).Append("\n");
            sb.Append("  DeliveryDate: ").Append(DeliveryDate).Append("\n");
            sb.Append("  CutoffTime: ").Append(CutoffTime).Append("\n");
            sb.Append("  ErrorMessage: ").Append(ErrorMessage).Append("\n");
            sb.Append("}\n");
            return sb.ToString();
        }

        /// <summary>
        /// Returns the JSON string presentation of the object
        /// </summary>
        /// <returns>JSON string presentation of the object</returns>
        public string ToJson()
        {
            return JsonConvert.SerializeObject(this, Formatting.Indented);
        }

        /// <summary>
        /// Returns true if objects are equal
        /// </summary>
        /// <param name="obj">Object to be compared</param>
        /// <returns>Boolean</returns>
        public override bool Equals(object obj)
        {
            if (ReferenceEquals(null, obj)) return false;
            if (ReferenceEquals(this, obj)) return true;
            return obj.GetType() == GetType() && Equals((VendorStocks)obj);
        }

        /// <summary>
        /// Returns true if VendorStocks instances are equal
        /// </summary>
        /// <param name="other">Instance of VendorStocks to be compared</param>
        /// <returns>Boolean</returns>
        public bool Equals(VendorStocks other)
        {
            if (ReferenceEquals(null, other)) return false;
            if (ReferenceEquals(this, other)) return true;

            return 
                (
                    Stocks == other.Stocks ||
                    Stocks != null &&
                    Stocks.SequenceEqual(other.Stocks)
                ) && 
                (
                    DeliveryDate == other.DeliveryDate ||
                    DeliveryDate != null &&
                    DeliveryDate.Equals(other.DeliveryDate)
                ) && 
                (
                    CutoffTime == other.CutoffTime ||
                    CutoffTime != null &&
                    CutoffTime.Equals(other.CutoffTime)
                ) && 
                (
                    ErrorMessage == other.ErrorMessage ||
                    ErrorMessage != null &&
                    ErrorMessage.Equals(other.ErrorMessage)
                );
        }

        /// <summary>
        /// Gets the hash code
        /// </summary>
        /// <returns>Hash code</returns>
        public override int GetHashCode()
        {
            unchecked // Overflow is fine, just wrap
            {
                var hashCode = 41;
                // Suitable nullity checks etc, of course :)
                    if (Stocks != null)
                    hashCode = hashCode * 59 + Stocks.GetHashCode();
                    if (DeliveryDate != null)
                    hashCode = hashCode * 59 + DeliveryDate.GetHashCode();
                    if (CutoffTime != null)
                    hashCode = hashCode * 59 + CutoffTime.GetHashCode();
                    if (ErrorMessage != null)
                    hashCode = hashCode * 59 + ErrorMessage.GetHashCode();
                return hashCode;
            }
        }

        #region Operators
        #pragma warning disable 1591

        public static bool operator ==(VendorStocks left, VendorStocks right)
        {
            return Equals(left, right);
        }

        public static bool operator !=(VendorStocks left, VendorStocks right)
        {
            return !Equals(left, right);
        }

        #pragma warning restore 1591
        #endregion Operators
    }
}
